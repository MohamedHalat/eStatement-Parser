...
class ByteSizeValue {
    constructor(valueInBytes) {
        ...
        if (!Number.isSafeInteger(valueInBytes) || valueInBytes < 0) {
            throw new Error(`Value in bytes is expected to be a safe positive integer, but provided [${valueInBytes}].`);
        }
    }
    static parse(text) {
        ...
        if (!match) {
            ...
            if (typeof number !== 'number' || isNaN(number)) {
                throw new Error(`Failed to parse [${text}] as byte value. Value must be either number of bytes, or follow the format <count>[b|kb|mb|gb] ` +
