...
function stringToDuration(text) {
    ...
    if (!result) {
        ...
        if (typeof number !== 'number' || isNaN(number)) {
            throw new Error(`Failed to parse [${text}] as time value. Value must be a duration in milliseconds, or follow the format ` +
...
function numberToDuration(numberMs) {
    if (!Number.isSafeInteger(numberMs) || numberMs < 0) {
        throw new Error(`Value in milliseconds is expected to be a safe positive integer, but provided [${numberMs}].`);
    }
