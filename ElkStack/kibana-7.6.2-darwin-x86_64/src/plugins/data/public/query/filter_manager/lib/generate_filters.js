...
function getExistingFilter(appFilters, fieldName, value) {
  ...
  return _lodash.default.find(appFilters, function (filter) {
    ...
    if (fieldName === '_exists_' && _common.esFilters.isExistsFilter(filter)) {
      return filter.exists.field === value;
    }

    if (_common.esFilters.isPhraseFilter(filter)) {
      return _common.esFilters.getPhraseFilterField(filter) === fieldName && _common.esFilters.getPhraseFilterValue(filter) === value;
    }

    if (_common.esFilters.isScriptedPhraseFilter(filter)) {
...
function generateFilters(filterManager, field, values, operation, index) {
  ...
  _lodash.default.each(values, function (value) {
    ...
    if (existing) {
      ...
      var filterType = fieldName === '_exists_' ? _common.esFilters.FILTERS.EXISTS : _common.esFilters.FILTERS.PHRASE;
      var actualFieldObj = fieldName === '_exists_' ? {
        name: value
      } : fieldObj;
      filter = _common.esFilters.buildFilter(tmpIndexPattern, actualFieldObj, filterType, negate, false, value, null, _common.esFilters.FilterStateStore.APP_STATE);
    }
