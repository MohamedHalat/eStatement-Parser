...
function (_Component) {
  ...
  function FilterEditorUI(props) {
    ...
    _defineProperty(_assertThisInitialized(_this), "onSubmit", function () {
      ...
      if (isCustomEditorOpen) {
        ...
        var _filter = _.esFilters.buildCustomFilter(newIndex, body, disabled, negate, alias, $state.store);

        _this.props.onSubmit(_filter);
      } else if (indexPattern && field && operator) {
        var _filter2 = _.esFilters.buildFilter(indexPattern, field, operator.type, operator.negate, _this.props.filter.meta.disabled, params, alias, $state.store);
    ...
    _this.state = {
      ...
      params: _.esFilters.getFilterParams(props.filter),
      useCustomLabel: props.filter.meta.alias !== null,
      customLabel: props.filter.meta.alias,
      queryDsl: JSON.stringify(_.esFilters.cleanFilter(props.filter), null, 2),
  ...
  _createClass(FilterEditorUI, [{
    ...
    value: function getIndexPatternFromFilter() {
      return _.esFilters.getIndexPatternFromFilter(this.props.filter, this.props.indexPatterns);
    }
  }, {
