...
var _ingest = require("./server/api_server/es_6_0/ingest");

var _server = require("./server/api_server/server");

var _spec = require("./server/api_server/spec");
...
function _default(kibana) {
  const npSrc = (0, _path.resolve)(__dirname, 'public/np_ready');
  let defaultVars;
  return new kibana.Plugin({
    ...
    async init(server, options) {
      ...
      server.expose('addProcessorDefinition', _ingest.addProcessorDefinition);
      ...
      const legacyEsConfig = await server.newPlatform.__internals.elasticsearch.legacy.config$.pipe((0, _operators.first)()).toPromise();
    ...
    uiExports: {
      devTools: [(0, _path.resolve)(__dirname, 'public/legacy')],
      styleSheetPaths: (0, _path.resolve)(npSrc, 'application/styles/index.scss'),
      injectDefaultVars: () => defaultVars,
      noParse: [(0, _path.join)(npSrc, 'application/models/legacy_core_editor/mode/worker/worker.js')]
    }
  });
}
