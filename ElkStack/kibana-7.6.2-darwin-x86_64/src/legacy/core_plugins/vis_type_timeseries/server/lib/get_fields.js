...
async function getFields(req) {
  ...
  const fields = (await searchStrategy.getFieldsForWildcard(req, indexPatternString, capabilities)).filter(field => field.aggregatable && !(0, _server.isNestedField)(field));
